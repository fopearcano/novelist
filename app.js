...
if (typeof document !== 'undefined') {
  ...
  function renameNode(node, label) { const value = prompt(label, node.title); if (value?.trim()) { node.title = value.trim(); renderOutline(); persist(); } }
  ...
  function editTag(id) { const tag = project.tags.find(item => item.id === id); const name = prompt('Element name', tag.name); if (!name?.trim()) return; const type = prompt('Type: Character, Place, Object, or Music / sound', tag.type); const color = prompt('Tag color (hex)', tag.color); if (['Character', 'Place', 'Object', 'Music / sound'].includes(type)) tag.type = type; if (/^#[0-9a-f]{6}$/i.test(color)) tag.color = color; tag.name = name.trim(); renderTags(); renderOutline(); persist(); }
  ...
  $('#confirmTag').addEventListener('click', () => { const name = $('#tagName').value.trim(); if (!name) return; const type = $('#tagType').value; const tag = { id: `tag-${Date.now()}`, name, type, color: $('#tagColor').value, icon: type === 'Music / sound' ? '♫' : '◆' }; project.tags.push(tag); currentScene().tags.push(tag.id); $('#tagName').value = ''; $('#tagDialog').close(); renderTags(); persist(true); });
