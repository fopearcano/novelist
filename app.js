...
if (typeof document !== 'undefined') {
  ...
  function renderOutline() {
    $('#outlineTree').innerHTML = project.acts.map((act, ai) => `<section class="outline-group"><div class="group-header"><button class="chevron" aria-label="Toggle act">▼</button><span>${escapeHTML(act.title)}</span><span class="tree-actions"><button data-edit-act="${ai}" title="Rename act">✎</button><button data-add-chapter="${ai}" title="Add chapter">＋</button><button data-delete-act="${ai}" title="Delete act">×</button></span></div><div class="group-content">${act.chapters.map((chapter, ci) => `<div class="chapter"><div class="chapter-header"><button class="chevron" aria-label="Toggle chapter">▼</button><span>${escapeHTML(chapter.title)}</span><span class="tree-actions"><button data-edit-chapter="${ai}:${ci}" title="Rename chapter">✎</button><button data-add-scene="${ai}:${ci}" title="Add scene">＋</button><button data-delete-chapter="${ai}:${ci}" title="Delete chapter">×</button></span></div><div class="scene-list">${chapter.scenes.map(item => `<div class="scene-item ${item.id === project.activeScene ? 'active' : ''} ${filterTag && !item.tags.includes(filterTag) ? 'filtered' : ''}" style="--scene-color:${item.color}"><button class="scene-open" data-scene="${item.id}"><span class="scene-name">${escapeHTML(item.title)}</span><span class="scene-count">${item.words || '—'}</span></button><span class="scene-actions-inline"><button data-edit-scene="${item.id}" title="Rename scene" aria-label="Rename ${escapeHTML(item.title)}">✎</button><button data-delete-scene="${item.id}" title="Delete scene" aria-label="Delete ${escapeHTML(item.title)}">×</button></span></div>`).join('')}</div></div>`).join('')}</div></section>`).join('');
    ...
    $$('.scene-open').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); openScene(button.dataset.scene); }));
    ...
    $$('[data-edit-scene]').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); const found = locateScene(button.dataset.editScene); if (found) renameNode(found.item, 'Scene name'); }));
    $$('[data-delete-scene]').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); deleteSceneById(button.dataset.deleteScene); }));
  }
  ...
  function addAct() { project.acts.push({ title: `ACT ${project.acts.length + 1} · UNTITLED`, chapters: [] }); renderOutline(); persist(true); }
  ...
  function deleteSceneById(id) { const found = locateScene(id); if (!found || allScenes(project).length === 1) return toast('A project needs at least one scene'); if (!confirm(`Delete “${found.item.title}”?`)) return; if (id === project.activeScene) captureScene(); const list = project.acts[found.ai].chapters[found.ci].scenes; list.splice(list.indexOf(found.item), 1); $('#sceneDialog').close(); if (id === project.activeScene) { const next = allScenes(project)[0]; project.activeScene = next.id; openScene(next.id, false); } else { renderOutline(); persist(); } toast('Scene deleted'); }
  function deleteScene() { deleteSceneById(project.activeScene); }
  ...
  $('#outlineAdd').addEventListener('click', () => $('#outlineAddDialog').showModal());
  $('#addActChoice').addEventListener('click', () => { $('#outlineAddDialog').close(); addAct(); });
  $('#addChapterChoice').addEventListener('click', () => { const found = locateScene(project.activeScene); $('#outlineAddDialog').close(); addChapter(found?.ai ?? 0); });
  $('#addSceneChoice').addEventListener('click', () => { const found = locateScene(project.activeScene); $('#outlineAddDialog').close(); addScene(found?.ai ?? 0, found?.ci ?? 0); });
  ...
  $('#backupProject').addEventListener('click', () => { captureScene(); persist(); downloadProject(project.id); $('#exportDialog').close(); });
  $('#topImportButton').addEventListener('click', () => $('#projectFile').click());
